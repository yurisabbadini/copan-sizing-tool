...
export default defineComponent({
  ...
  setup() {
    ...
    const numberOfIncubators = (lines.value.map((x) => x.numberOfCO2Incubator).reduce((a,b) => a+b)) + (lines.value.map((x) => x.numberOfO2Incubator).reduce((a,b) => a+b));
    ...
    primaryProtocols.value.forEach(protocol => {
      for(let i = 0; i < protocol.volumes.length; i++) {
        ...
        if(waspTimesData[i] > 3600 * (lines.value.map((x) => x.numberOfWasp).reduce((a, b) => a+b) || 1)) {
          let next = i+1;
          if(next > waspTimesData.length - 1) {
            next = 0;
          }
          waspTimesData[next] += waspTimesData[i] - (3600 * (lines.value.map((x) => x.numberOfWasp).reduce((a, b) => a+b) || 1));
          waspTimesData[i] = 3600 * (lines.value.map((x) => x.numberOfWasp).reduce((a, b) => a+b) || 1);
        }
      }
    });
    ...
    let totalColibriPlates = 0;
    lines.value.forEach(line => {
      line.protocols.forEach(protocol => {
        const primaryProtocol = primaryProtocols.value.find((x) => x.id == protocol.id);
        if(primaryProtocol) {
          const factor = primaryProtocol.idPercentage + primaryProtocol.astPercentage + primaryProtocol.astIdPercentage;
          totalColibriPlates += (protocol.co2Unloading + protocol.o2Unloading) * (factor / 100);
        }
      });
    });
    const colibriAvgTime = colibriTime.value / totalColibriPlates;

    for(let i = 0; i < unloadingPositiveChartData.length; i++) {
      unloadingPositiveChartData[i] = Number(((3600 * unloadingPositiveChartData[i] / 100)  / 3600 * (numberOfIncubators >= 1 ? settings.value.incubator.unloadingMultiplePlatesPerHour : settings.value.incubator.unloadingSinglePlatesPerHour) * colibriAvgTime / 3600 * 100).toFixed(2));
      if(unloadingPositiveChartData[i] > 3600) {
          let next = i+1;
          if(next > unloadingPositiveChartData.length - 1) {
            next = 0;
          }
          unloadingPositiveChartData[next] += unloadingPositiveChartData[i] - 3600;
          unloadingPositiveChartData[i] = 3600;
        }
    }
